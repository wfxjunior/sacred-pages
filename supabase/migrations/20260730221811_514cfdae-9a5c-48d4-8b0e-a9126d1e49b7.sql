create type public.journey_session_status as enum (
  'not_started','in_progress','puzzle_completed','completed','abandoned'
);
create type public.journey_step_type as enum (
  'scripture','devotional','puzzle','reflection','prayer','completion'
);
create type public.favorite_entity as enum ('collection', 'journey');
create type public.milestone_category as enum (
  'journey','puzzle','consistency','collection','reflection','prayer','discovery'
);
create type public.milestone_criteria as enum (
  'first_occurrence','cumulative_count','consistency_days','collection_completed','difficulty_reached'
);
create type public.activity_event_type as enum (
  'journey_started','journey_resumed','journey_step_completed','puzzle_started',
  'puzzle_completed','reflection_saved','prayer_saved','journey_completed',
  'collection_completed','favorite_added','favorite_removed','active_day_recorded','milestone_earned'
);
...
create table public.journey_sessions (
  ...
  constraint journey_sessions_completed_state check ((status = 'completed') = (completed_at is not null)),
  constraint journey_sessions_abandoned_state check ((status = 'abandoned') = (abandoned_at is not null))
);
...
create index journey_sessions_history_idx
  on public.journey_sessions (user_id, completed_at desc) where status = 'completed';
...
create index journey_step_progress_session_idx on public.journey_step_progress (session_id, step_order);
...
-- ============================ PROGRESS FUNCTIONS ==========================
create or replace function public.recalculate_collection_progress(uid uuid, target_collection uuid)
...
begin
  ...
  where j.primary_collection_id = target_collection
    and j.status = 'published' and j.archived_at is null;
...
create or replace function public.record_active_day(
  uid uuid, moment timestamptz default now(), completed_journey boolean default true
)
...
begin
  ...
  values
    (new.user_id, 'journey_completed', 'journey', new.journey_id,
     jsonb_build_object(
       'sessionId', new.id, 'elapsedMs', new.elapsed_ms, 'isReplay', new.is_replay,
       'difficulty', new.difficulty, 'languageCode', new.language_code
     ),
...
returns table (active_days int, current_run int, longest_run int, last_active date)
...
revoke execute on function public.recalculate_collection_progress(uuid, uuid) from public;
revoke execute on function public.record_active_day(uuid, timestamptz, boolean) from public;
revoke execute on function public.evaluate_milestones(uuid, public.activity_event_type, uuid) from public;
revoke execute on function public.user_local_date(uuid, timestamptz) from public;
revoke execute on function public.consistency_summary(uuid) from public;
grant execute on function public.evaluate_milestones(uuid, public.activity_event_type, uuid) to authenticated, service_role;
grant execute on function public.consistency_summary(uuid) to authenticated, service_role;
grant execute on function public.user_local_date(uuid, timestamptz) to authenticated, service_role;
grant execute on function public.record_active_day(uuid, timestamptz, boolean) to service_role;
grant execute on function public.recalculate_collection_progress(uuid, uuid) to service_role;
...
grant select, insert, update on public.journey_step_progress to authenticated;
grant select on public.journey_progress to authenticated;
...
grant select on public.consistency_days to authenticated;
grant select on public.milestone_definitions to anon, authenticated;
grant insert, update, delete on public.milestone_definitions to authenticated;
grant select on public.milestone_translations to anon, authenticated;
grant insert, update, delete on public.milestone_translations to authenticated;
grant select, update on public.user_milestones to authenticated;
grant select, insert on public.activity_events to authenticated;
grant select on public.user_collection_progress to authenticated;

grant all on public.journey_sessions, public.journey_step_progress, public.journey_progress,
  public.user_reflections, public.user_prayers, public.favorites, public.consistency_days,
  public.milestone_definitions, public.milestone_translations, public.user_milestones,
  public.activity_events, public.user_collection_progress
  to service_role;
...
create policy "users read own journey sessions"
  on public.journey_sessions for select to authenticated using (auth.uid() = user_id);
create policy "users create own journey sessions"
  on public.journey_sessions for insert to authenticated with check (auth.uid() = user_id);
...
create policy "users delete own journey sessions"
  on public.journey_sessions for delete to authenticated using (auth.uid() = user_id);

create policy "users read own step progress"
  on public.journey_step_progress for select to authenticated using (auth.uid() = user_id);
create policy "users write own step progress"
  on public.journey_step_progress for insert to authenticated
  with check (
    auth.uid() = user_id
    and exists (
...
create policy "users update own step progress"
  on public.journey_step_progress for update to authenticated
  using (auth.uid() = user_id) with check (auth.uid() = user_id);

create policy "users read own journey progress"
  on public.journey_progress for select to authenticated using (auth.uid() = user_id);
...
create policy "users create own reflections"
  on public.user_reflections for insert to authenticated with check (auth.uid() = user_id);
...
create policy "users delete own reflections"
  on public.user_reflections for delete to authenticated using (auth.uid() = user_id);

create policy "users read own prayers"
  on public.user_prayers for select to authenticated
  using (auth.uid() = user_id and deleted_at is null);
create policy "users create own prayers"
  on public.user_prayers for insert to authenticated with check (auth.uid() = user_id);
create policy "users update own prayers"
  on public.user_prayers for update to authenticated
  using (auth.uid() = user_id) with check (auth.uid() = user_id);
create policy "users delete own prayers"
  on public.user_prayers for delete to authenticated using (auth.uid() = user_id);
...
create policy "users read own consistency"
  on public.consistency_days for select to authenticated using (auth.uid() = user_id);
...
create policy "super admins manage milestone definitions"
  on public.milestone_definitions for all to authenticated
...
create policy "super admins manage milestone translations"
  on public.milestone_translations for all to authenticated
  using (public.has_role(auth.uid(), 'super_admin'))
  with check (public.has_role(auth.uid(), 'super_admin'));
...
create policy "users read own activity events"
  on public.activity_events for select to authenticated using (auth.uid() = user_id);
create policy "users append own activity events"
  on public.activity_events for insert to authenticated with check (auth.uid() = user_id);

create policy "users read own collection progress"
  on public.user_collection_progress for select to authenticated using (auth.uid() = user_id);

-- ============================ FK INDEXES (0008) ===========================
create index if not exists journey_step_progress_user_idx on public.journey_step_progress (user_id);
create index if not exists puzzle_attempts_user_idx on public.puzzle_attempts (user_id);
create index if not exists favorites_journey_fk_idx on public.favorites (journey_id);
create index if not exists journey_progress_journey_fk_idx on public.journey_progress (journey_id);
create index if not exists puzzle_sessions_journey_fk_idx on public.puzzle_sessions (journey_id);
create index if not exists user_reflections_journey_fk_idx on public.user_reflections (journey_id);
create index if not exists user_prayers_journey_fk_idx on public.user_prayers (journey_id);
create index if not exists favorites_collection_fk_idx on public.favorites (collection_id);
create index if not exists journey_progress_collection_fk_idx on public.journey_progress (collection_id);
create index if not exists journey_sessions_collection_fk_idx on public.journey_sessions (collection_id);
create index if not exists user_collection_progress_collection_fk_idx on public.user_collection_progress (collection_id);
create index if not exists user_reflections_session_fk_idx on public.user_reflections (session_id);
create index if not exists user_prayers_session_fk_idx on public.user_prayers (session_id);
create index if not exists journey_sessions_puzzle_instance_fk_idx on public.journey_sessions (puzzle_instance_id);
create index if not exists puzzle_progress_instance_fk_idx on public.puzzle_progress (puzzle_instance_id);
create index if not exists puzzle_generation_requests_result_fk_idx on public.puzzle_generation_requests (result_instance_id);
create index if not exists puzzle_generation_requests_template_fk_idx on public.puzzle_generation_requests (template_id);
create index if not exists collections_cover_media_fk_idx on public.collections (cover_media_id);
create index if not exists collections_thumbnail_media_fk_idx on public.collections (thumbnail_media_id);
create index if not exists journeys_hero_media_fk_idx on public.journeys (hero_media_id);
create index if not exists journeys_social_media_fk_idx on public.journeys (social_media_id);
create index if not exists user_milestones_milestone_fk_idx on public.user_milestones (milestone_id);
create index if not exists journey_words_scripture_reference_fk_idx on public.journey_words (scripture_reference_id);